...
export default async function (host: Tree, schema: InterfaceSchema) {
  ...
  // generate interfaces into app/my-app-name/lib/src/types
  const targetPath = path.join(project.sourceRoot, 'types');

  // read files from tools/generators/interface/files
  const templatePath = 'tools/generators/interface/files';
